...
def modelfunc2(t, k1, k2, k3, k4, k5, k6, k7, y0, t_dark):
    global sol1, sol2, sol3
    sol1 = solve_ivp(kinetic, [t[0], t[t_dark]], [0, 0, 0, y0], t_eval=t[0:t_dark],
                     args=[k1, k2, k3, k4, k5, k6, k7])
    sol2 = solve_ivp(kinetic, [t[t_dark], t[-1]], sol1.y[:, -1], t_eval=t[t_dark:-1],
                     args=[0, k2, k3, k4, 0, k6, k7])
...
p0_control = [1 / 2, 1 / 2, 1 / 1, 1/120, 1/1, 1 / 100, 0, 1, 0]
p0_gco = [1 / 60, 1 / 20, 1 / 20, 1, 1, 1 / 100, 1 / 120, 1, 0]
...
def partint(dataset, partnum=1, p0=p0_control):
    ...
    def fitfunc(t, k1, k2, k3, k4, k5, k6, k7, y0, t_dark_offset):
        return modelfunc2(t, k1, k2, k3, k4, k5, k6, k7, y0, t_dark+int(t_dark_offset*1e9))

    popt, pcov = curve_fit(fitfunc, t_plot, norm_pulsephotons, p0=p0, bounds=([0, 0, 0, 0, 0, 0, 0, 0, -40e-9],
                                   [np.inf, 2, np.inf, 0.02, np.inf, np.inf, np.inf, np.inf, 200e-9]))

    # popt = [1/60, 1/2, 1/190, 1/120, 1/5, 1/20, 1, 1, 0]
    tau1 = 1 / popt[0]
    tau2 = 1 / popt[1]
    tau3 = 1 / popt[2]
    tau4 = 1 / popt[3]
...
c2380, c2380fit, t_c2380 = partint(dataset_2380, 1)
