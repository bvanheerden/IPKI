...
def kinetic(t, y, k1, k2, k3, k4, k5, k7):
    K = np.array([[-k4, k3,    k7, k5],
                  [0,  -k2-k3, 0,  k1],
                  [0,   0,    -k7, 0],
                  [k4,  k2,    0, -k1-k5]])
    return K @ y
...
def modelfunc2(t, k1, k2, k3, k4, k5, k7, y0, q0, t_dark):
    global sol1, sol2, sol3
    sol1 = solve_ivp(kinetic, [t[0], t[t_dark]], [0, 0, q0, y0], t_eval=t[0:t_dark], args=[k1, k2, k3, k4, 0, k7])
    sol2 = solve_ivp(kinetic, [t[t_dark], t[-1]], sol1.y[:, -1], t_eval=t[t_dark:-1], args=[0, k2, k3, k4, k5, 0])
...
p0_fast = [1 / 3.63, 1 / 0.64, 1 / 2.6, 1 / 60, 1 / 100, 1 / 0.6, 0.5, 0.5, 0]
...
def partint(dataset, partnum=1, p0=p0_fast):
    ...
    def fitfunc(t, k1, k2, k3, k4, k5, k7, y0, q0, t_dark_offset):
        return modelfunc2(t, k1, k2, k3, k4, k5, k7, y0, q0, t_dark+int(t_dark_offset*1e9))

    popt, pcov, *extra = curve_fit(fitfunc, t_plot, norm_pulsephotons,
                                   p0=p0, bounds=([0, 0, 1/5, 0, 0,1, 0, 0, -40e-9],
                                   [np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, 10e-9]),
                                   verbose=1, xtol=2e-16)

    # popt = [0.3, 1, 0.05, 0.01, 1, 1, 0.000001, 1, 0, 0]
    # popt = [1 / 3.63, 1 / 0.64, 1 / 2.6, 1 / 60, 1 / 100, 1 / 800, 1/0.6, 0.5, 0.5, 0]
    tau1 = 1 / popt[0]
    tau2 = 1 / popt[1]
    tau3 = 1 / popt[2]
    tau4 = 1 / popt[3]
...
eight, eightfit, t_eight = partint(dataset, 3, p0=p0_fast)
...
ax2.plot(t_eight, eightfit-eight, label='Residuals')
