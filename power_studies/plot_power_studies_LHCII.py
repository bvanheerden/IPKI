...
try:
    with open('data_no_aa_lhcii.pkl', 'rb') as f:
        df_no_aa = pickle.load(f)
except FileNotFoundError:
    df_no_aa = pd.DataFrame()
    print("data_no_aa_k2_lhcii.pkl not found")

try:
    with open('data_with_aa_lhcii.pkl', 'rb') as f:
        df_with_aa = pickle.load(f)
except FileNotFoundError:
    df_with_aa = pd.DataFrame()
    print("data_with_aa_k2_lhcii.pkl not found")

try:
    with open('data_no_aa.pkl', 'rb') as f:
        df_thylakoid_no_aa = pickle.load(f)
except FileNotFoundError:
    print("data_no_aa.pkl not found")
...
if not df_no_aa.empty:
    ...
    if not df_thylakoid_no_aa.empty:
        ...
        ax1.errorbar(x_thy_no_aa, k3_thy,
                     ...
                     linewidth=1, markersize=4, alpha=0.3, color='C1', capsize=3, label=None)
        m3_thy = np.sum(x_thy_no_aa[:] * k3_thy[:]) / np.sum(x_thy_no_aa[:]**2)
        ax1.plot(x_extrap, m3_thy * x_extrap, 'C1--', alpha=0.3, label=None)
        ...
        ax1.errorbar(x_thy_no_aa, k4_thy,
                     ...
                     linewidth=1, markersize=4, alpha=0.3, color='C2', capsize=3, label=None)
        m4_thy = np.sum(x_thy_no_aa[:] * k4_thy[:]) / np.sum(x_thy_no_aa[:]**2)
        ax1.plot(x_extrap, m4_thy * x_extrap, 'C2--', alpha=0.3, label=None)
        ...
        ax1.errorbar(x_thy_no_aa, k1_thy,
                     ...
                     linewidth=1, markersize=4, alpha=0.3, color='C0', capsize=3, label=None)
        m1_thy = np.sum(x_thy_no_aa[:-3] * k1_thy[:-3]) / np.sum(x_thy_no_aa[:-3]**2)
        ax1.plot(x_extrap, m1_thy * x_extrap, 'C0--', alpha=0.3, label=None)
