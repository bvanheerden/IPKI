...
if all_datasets:
    ...
    df.to_csv(os.path.join(results_dir, 'mixed_effects_results.csv'), index=False)
    
    df_no_aa = df[df['AA'] == 'No'].sort_values('Power (mE)')
    df_with_aa = df[df['AA'] == 'Yes'].sort_values('Power (mE)')
    
    with open(os.path.join(results_dir, 'data_no_aa_lhcii.pkl'), 'wb') as f:
        pickle.dump(df_no_aa, f)
    with open(os.path.join(results_dir, 'data_with_aa_lhcii.pkl'), 'wb') as f:
