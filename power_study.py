...
def fitfunc(t, k1, k2, k3, k4, k5, y0, t_dark_offset):
    return modelfunc2(t, k1, 1/1.3, 1/2.5, 0, k5, y0, t_dark+int(t_dark_offset*1e9))


# dataset = h5py.File('blinking/Power study.h5', 'r')
dataset = h5py.File('blinking/3000 mE.h5', 'r')

abstimes = dataset['Particle 4']['Absolute Times (ns)']
...
# t_plot = np.linspace(0, 50, 1870)
# t = np.linspace(0, 50, 187)
# t_plot = np.linspace(0, 70, 2650)
# t = np.linspace(0, 70, 265)
# t_plot = np.linspace(0, 110, 4160)
# t = np.linspace(0, 110, 416)
# t_plot = np.linspace(0, 190, 7460)
# t = np.linspace(0, 190, 746)

norm_pulsephotons = np.interp(t_plot, t[:-1], norm_pulsephotons)[:-1]
popt, pcov = curve_fit(fitfunc, t_plot, norm_pulsephotons, p0=[1/30, 1/1.3, 1/1.8, 1, 1/40, 1200, 0],
                       bounds=([0, 0, 0, 0, 0, 0, -40e-9],
                               [np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, 30e-9]))

# popt = [1/30, 1/1.3, 1/1.8, 1, 1/40, 1200, 0]
print(popt)
