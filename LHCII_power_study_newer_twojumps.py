...
p0 = [1 / 20, 1 / 0.7, 1 / 0.5, 1 / 0.5, 1 / 50, 1 / 2.61, 1/1, 1/1, 0.8, 0.2]
...
def fittrace(dataset, partnums):
    ...
    def fitfunc(t, k1, k2, k3, k4, k5, k6, k7, k8, y0, q0):
        sol1, sol2, sol3, sol4 = modelfunc(t, k1, k2, k3, k4, k5, k6, k7, k8, y0, q0, t_dark, t_light, t_dark2)#+int(t_dark_offset*1e10))
    ...
    popt, pcov, *extra = curve_fit(fitfunc, t, norm_pulsephotons, p0=p0, bounds=([0, 0, 0, 0, 0, 0, 0, 0, 0.5, 0],
                                   [np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, np.inf, 1]),
                                   verbose=2)
...
norm_pulsephotons_150, model_150, *params_150 = fittrace(dataset_150, [1, 2, 3])
...
# #
# norm_pulsephotons_760, model_760, *stuff = fittrace(dataset_760, [1, 2, 3])
# plt.plot(norm_pulsephotons_760[:], '--')
# plt.plot(model_760[:], label='760 uW', color='C4')
# #
# norm_pulsephotons_1140, model_1140, *stuff = fittrace(dataset_1140, [1, 2, 3])#, 11, 12, 13, 14, 15])
# plt.plot(norm_pulsephotons_1140[:], '--')
# plt.plot(model_1140[:], label='1140 uW', color='C5')
# #
# # fig, (ax1, ax2) = plt.subplots(2, 1, height_ratios=(2, 1))
norm_pulsephotons_1700, model_1700, t_1700, *params_1700 = fittrace(dataset_1700, [1, 2, 3])#, 11, 12, 13, 14, 15])
